...
if [ $stage -le 10 ]; then
  for test in dev_clean_5; do
    steps/decode_biglm_fast.sh --stage 2 --nj 38 --cmd "$decode_cmd" \
    --online-ivector-dir exp/nnet3/ivectors_${test}_hires \
    --beam 15.0 --lattice-beam 8.0 --expand-beam 15.0 \
    exp/chain/tree_sp/graph_tgsmall \
    data/lang_test_tgsmall/G.fst data/lang_test_tglarge/G.fst \
    data/${test}_hires \
    exp/chain/tdnnopgru_1a_sp/decode_biglm2_${test}_diff_15_8_15_cutoff
  done
fi
