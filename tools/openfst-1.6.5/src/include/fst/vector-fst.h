...
class VectorFstImpl : public VectorFstBaseImpl<S> {
 public:
  ...
  StateId AddState(State *istate) {
    const auto state = BaseImpl::AddState(istate);
    SetProperties(AddStateProperties(Properties()));
    return state;
  }
...
class VectorFst : public ImplToMutableFst<internal::VectorFstImpl<S>> {
 public:
  ...
  StateId AddState(State *istate = NULL) {
      if (!istate) GetMutableImpl()->AddState();
      return GetMutableImpl()->AddState(istate);
  }
