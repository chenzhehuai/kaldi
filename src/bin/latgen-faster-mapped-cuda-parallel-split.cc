...
int main(int argc, char *argv[]) {
  try {
    ...
    int feat_start = 3;
    int feat_end = po.NumArgs() - 3;
    std::string model_in_filename = po.GetArg(1),
                fst_in_str = po.GetArg(2),
                
                lattice_wspecifier = po.GetArg(po.NumArgs()-2),
                words_wspecifier = po.GetArg(po.NumArgs()-1),
                alignment_wspecifier = po.GetArg(po.NumArgs());
    ...
    double elapsed_min = 1e5;

    std::vector<std::string> feature_vector;
    int num_feat = feat_end - feat_start + 1;
    assert(num_feat <= 20);
    omp_set_num_threads(num_feat);
    KALDI_VLOG(0) << "Set thread num according to feat num: " << num_feat;
    feature_vector.reserve(num_feat);
      ...
      #pragma omp parallel shared(po, decode_fst_cuda)
      {
        ...
        */
        ...
        LatticeFasterDecoderCuda decoder(decode_fst_cuda, config);
        {
          ...
          #pragma omp critical
          {
            elapsed_max = std::max(elapsed_max, t_local);
            elapsed_min = std::min(elapsed_min, t_local);
          }
        }
        #pragma omp barrier
      }//omp parallel
    ...
    KALDI_LOG << "Time taken " << elapsed
              ...
              << (elapsed_max * 100.0 / frame_count)  <<"\t"
              << (elapsed_min * 100.0 / frame_count);
