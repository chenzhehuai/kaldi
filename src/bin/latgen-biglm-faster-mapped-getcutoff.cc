...
bool DecodeUtterance(LatticeBiglmFasterDecoder &decoder, // not const but is really an input.
                     ...
                     BaseFloatVectorWriter *cutoff_writer,
  ...
  // Get cutoff
  {
    Vector<BaseFloat> cutoff;
    decoder.GetExactCutoffPerFrame(&cutoff);
    if (cutoff_writer->IsOpen()) {
      cutoff_writer->Write(utt, cutoff);
    }
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: latgen-biglm-faster-mapped-getcutoff [options] model-in (fst-in|fsts-rspecifier) "
        "oldlm-fst-in newlm-fst-in features-rspecifier"
        " lattice-wspecifier cutoff-wspecifier [ words-wspecifier [alignments-wspecifier] ]\n";
    ...
    std::string model_in_filename = po.GetArg(1),
        ...
        cutoff_wspecifier = po.GetArg(7),  // get cutoff
    ...
    if (! (determinize ? compact_lattice_writer.Open(lattice_wspecifier)
      ...
      KALDI_ERR << "Could not open table for writing lattices: "
                 << lattice_wspecifier;
    BaseFloatVectorWriter cutoff_writer(cutoff_wspecifier);  // get cutoff
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          if (DecodeUtterance(decoder, decodable, trans_model, word_syms,
                              ...
                              &compact_lattice_writer, &lattice_writer, &cutoff_writer,
                              &like)) {
            tot_like += like;
