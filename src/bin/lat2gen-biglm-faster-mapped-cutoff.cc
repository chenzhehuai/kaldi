...
bool DecodeUtterance(Lattice2BiglmFasterDecoder &decoder, // not const but is really an input.
  ...
  if (!decoder.Decode(&decodable, cutoff)) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: lat2gen-biglm-faster-mapped-cutoff [options] model-in (fst-in|fsts-rspecifier) "
        "oldlm-fst-in newlm-fst-in features-rspecifier cutoff-rspecifier "
    ...
    RandomAccessBaseFloatVectorReader cutoff_reader(cutoff_rspecifier);
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          Vector<BaseFloat> cutoff(cutoff_reader.Value(utt));
          if (cutoff.Dim() == 0) {
            KALDI_ERR << "No corresponding cutoff vector: " << utt;
            num_fail++;
            continue;
          }
          ...
          double like;
          if (DecodeUtterance(decoder, decodable, cutoff, trans_model, word_syms,
