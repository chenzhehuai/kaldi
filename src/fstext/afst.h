...
namespace afst {
  ...
  #define GET_LOWER_BITS(v,n) ((1<<(n))-1)&v  

  //hfst_id 14 //afst_id 8 //disambig_sym 10
  inline void EncodeIlabel(uint32 &ilabel, const uint16 hfst_id, 
                          const uint16 afst_id, const uint16 disambig_sym) {
    ilabel = hfst_id;
    ilabel=(ilabel<<8)+afst_id;
    ilabel=(ilabel<<10)+disambig_sym;
  }
  inline void DecodeIlabel(const uint32 ilabel, uint16 &hfst_id, 
                          uint16 &afst_id, uint16 &disambig_sym) {
