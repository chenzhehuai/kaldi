...
#ifndef FSTEXT_ENCODE_H_
#define FSTEXT_ENCODE_H_
...
class EncodeTable {
 public:
  ...
  using EncodeHash = concurrent_unordered_map<const Tuple *, Label, TupleKey,
                                        TupleEqual>;
...
#pragma omp critical
  ...
  void SetInputSymbols(const SymbolTable *syms) {
    if (syms) {
      ...
      flags_ |= internal::kEncodeHasISymbols;
    } else {
      isymbols_.reset();
      flags_ &= ~internal::kEncodeHasISymbols;
    }
  ...
  void SetOutputSymbols(const SymbolTable *syms) {
    if (syms) {
      ...
      flags_ |= internal::kEncodeHasOSymbols;
    } else {
      osymbols_.reset();
      flags_ &= ~internal::kEncodeHasOSymbols;
    }
 ...
 private:
  uint32 flags_;
  concurrent_vector<std::unique_ptr<Tuple>> encode_tuples_;
...
bool EncodeTable<Arc>::Write(std::ostream &strm,
  ...
  if (flags_ & internal::kEncodeHasISymbols) isymbols_->Write(strm);
  if (flags_ & internal::kEncodeHasOSymbols) osymbols_->Write(strm);
  strm.flush();
  if (!strm) {
...
EncodeTable<Arc> *EncodeTable<Arc>::Read(std::istream &strm,
  ...
  if (flags & internal::kEncodeHasISymbols) {
    table->isymbols_.reset(SymbolTable::Read(strm, source));
  }
  if (flags & internal::kEncodeHasOSymbols) {
...
class EncodeMapper {
 ...
 public:
  EncodeMapper(uint32 flags, EncodeType type)
      : flags_(flags),
        type_(type),
        table_(std::make_shared<internal2::EncodeTable<Arc>>(flags)),
        error_(false) {}
  ...
  static EncodeMapper<Arc> *Read(std::istream &strm, const string &source,
    ...
    auto *table = internal2::EncodeTable<Arc>::Read(strm, source);
 ...
 private:
  uint32 flags_;
  EncodeType type_;
  std::shared_ptr<internal2::EncodeTable<Arc>> table_;
  bool error_;

  explicit EncodeMapper(uint32 flags, EncodeType type,
                        internal2::EncodeTable<Arc> *table)
