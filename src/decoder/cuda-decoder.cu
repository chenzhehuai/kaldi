...
CudaDecoder::CudaDecoder(const CudaFst &fst, const TransitionModel &trans_model, const CudaDecoderConfig &config): fst_(fst), beam_(config.beam),
  bytes_cudaMalloc(0), max_tokens(config.max_tokens), trans_model_(trans_model),
  config_(config) {
...
__global__ void init_lookup_kernel(uint64 *d_state_cost, int size) {
    for(int idx = blockIdx.x*blockDim.x + threadIdx.x;
        ...
        d_state_cost[idx]  = pack_cost_idx_into_uint64(FLT_MAX,-1);
    }
}
...
DEVICE void reset_lookup_kernel(StateId *d_q, int *d_q_offset, int *d_q_end, uint64 *d_state_cost, float *d_cutoff, int *d_dbg_tok_num, int frame, int* d_q_token_from_narcs, bool reset=true) {
    ...
    for(int idx = q_offset + blockIdx.x*blockDim.x + threadIdx.x;
        ...
        d_state_cost[state]  = pack_cost_idx_into_uint64(FLT_MAX, -1);
    }
       ...
       __shared__ int is_last_CTA;
        ...
        for(int block_offset = blockDim.x*blockIdx.x;
            ...
            if(idx < queue_end) {
                ...
                if(cost < cutoff) {
                    int ptr= unpack_idx_from_uint64(d_state_cost[state_idx]);
                    if(ptr == idx) {
...
void __global__ get_cutoff(ExpandArcParams params, BaseFloat set = 0) {
    ...
    for(int block_offset = blockDim.x*blockIdx.x;
        ...
        if(valid_input) {
            ...
            BaseFloat accoustic_cost = (arc_ilabel != 0) ? -params.cuda_decodable.LogLikelihood(
                       arc_ilabel): 0.0; 
...
void __global__ expand_arcs_kernel(ExpandArcParams params) {
    ...
    for(int block_offset = blockDim.x*blockIdx.x;
        ...
        if(has_successor) {
            ...
            atomicMin((unsigned long long *)&params.d_lookup[arc_next_state], (unsigned long long)pack_cost_idx_into_uint64(total_cost, new_q_index));
        }
    }
...
uint64 *d_state_cost, BaseFloat *d_final, bool final) {
    ...
    for(int idx = q_offset + blockIdx.x*blockDim.x + threadIdx.x;
        ...
        d_state_cost[idx-q_offset] = pack_cost_idx_into_uint64(cost,idx);
    }
...
void CudaDecoder::DecodeChunk(CuMatrix<BaseFloat> *post_chunk) {
  ...
  num_frames_decoded_++;
    ...
    cudaStreamWaitEvent(compute_st, event_ll, 0);
